...
아래 스크립트를 실행하여 경로/아키텍처 변수를 결정한다. 이후 모든 Step에서 이 변수들을 사용한다.

**① GBS 빌드루트 기본 경로**

`~/.gbs.conf`의 `[general]` 섹션 `buildroot` 값을 사용하고, 없으면 기본값(`~/GBS-ROOT`)을 사용한다.
...
에뮬레이터면 `x86_64`, 실제 디바이스면 설치된 RPM에서 자동 감지한다.

```bash
DEVICE_TYPE="emulator"   # 또는 "device"
SDB="sdb -e"             # 에뮬레이터: "sdb -e", 실제 디바이스: "sdb -d"

if [ "${DEVICE_TYPE}" = "emulator" ]; then
    ARCH="x86_64"
else
    # noarch 제외하고 aarch64, armv7l 등 실제 arch 추출
...
```bash
cd "${SYSTEMD_SRC}"   # 예: /home/choyj/workspace/systemd-optimization/systemd
...
빌드 결과 RPM 위치 (`${RPMS}`):
```
${GBS_BUILDROOT_BASE}/local/repos/tizen/${ARCH}/RPMS/
├── systemd-244-0.${ARCH}.rpm          # 메인 패키지 (systemd 바이너리)
├── libsystemd-244-0.${ARCH}.rpm       # 공유 라이브러리
├── systemd-debuginfo-244-0.${ARCH}.rpm
└── ...
```
...
# 에뮬레이터 바이너리 해시 계산
EMU_HASH=$(${SDB} shell "md5sum /usr/lib/systemd/systemd" | awk '{print $1}')
...
```bash
# RPMS, ARCH, SDB 변수는 "변수 설정" 섹션에서 이미 설정됨

# 1. 에뮬레이터 연결 확인
sdb devices
# 출력 예: emulator-26101    device    T-10.0-x86_64
...
```bash
# gcov 래퍼 스크립트 생성 (BUILDROOT 변수는 "변수 설정" 섹션에서 결정됨)
# ld-linux-x86-64.so.2는 GBS 빌드루트 바이너리를 호스트에서 실행하기 위한 것이므로 x86_64 고정
...
# GCNO_BASE, BUILDROOT: "변수 설정" 섹션에서 결정됨
SRC="${SYSTEMD_SRC}"   # 예: /home/choyj/workspace/systemd-optimization/systemd
...
| `${RPMS}/` | GBS 빌드 결과 RPM (`${GBS_BUILDROOT_BASE}/local/repos/tizen/${ARCH}/RPMS/`) |
| `${BUILDROOT}/` | GBS 빌드루트 (`${GBS_BUILDROOT_BASE}/local/BUILD-ROOTS/scratch.${ARCH}.0`) |
