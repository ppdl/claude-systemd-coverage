...
GBS 빌드루트 경로 (이하 `BUILDROOT`):
```
BUILDROOT=/home/${USER}/GBS-ROOT/local/BUILD-ROOTS/scratch.x86_64.0
```

GBS 빌드 결과 RPM 경로 (이하 `RPMS`):
```
RPMS=/home/${USER}/GBS-ROOT/local/repos/tizen/x86_64/RPMS
```

GBS 빌드 내부 gcno 기준 경로 (이하 `GCNO_BASE`):
```
...
빌드 성공 확인:
```bash
grep "b_coverage" ${RPMS}/../logs/success/systemd-244-0/log.txt
...
# RPM 전송
${SDB} push "${RPMS}/libsystemd-244-0.x86_64.rpm" /tmp/
${SDB} push "${RPMS}/systemd-244-0.x86_64.rpm" /tmp/

# 설치
${SDB} shell "rpm -Uvh --force /tmp/libsystemd-244-0.x86_64.rpm /tmp/systemd-244-0.x86_64.rpm"
